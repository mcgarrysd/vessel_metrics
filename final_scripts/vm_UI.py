    ...
    if file_type == 'czi':
        ...
        if roi_yn == True:
            for s,t,u in zip(images_to_analyze, file_names, seg_list):
                ...
                full_output_dir = os.path.join(output_path,file_name)
                cropped_im,r = vm.select_roi(s,full_output_dir,this_slice)
        ...
        if roi_yn == True:
            full_output_dir = os.path.join(output_path,file_name)
            cropped_im, r = vm.select_roi(img,full_output_dir)
    ...
    if len(set(file_types)) == 1:
        ...
        if roi_yn == True:
            for s,t,u in zip(images_to_analyze, file_names, seg_list):
                this_file = t.split('_slice')[0]
                this_slice = t.split('_')[-1]
                full_output_dir = os.path.join(output_path,this_file)
                cropped_im, r = vm.select_roi(s,full_output_dir)
                vm.parameter_analysis_roi(s,u,params,output_path,t,r)
    else:
